      ...
      <div className="min-h-dvh w-full bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden font-sans select-none selection:bg-accent">
        ...
        <div className="w-full max-w-2xl bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative z-10 flex flex-col items-center text-center">
          ...
          <div className="relative mb-6">
            <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} className="p-6 bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-3xl shadow-xl">
              <Hammer size={40} />
            </motion.div>
          ...
          <h1 className="text-2xl md:text-4xl font-black mb-4">{isRtl ? "نقوم ببعض التحديثات السحرية" : "Mise à niveau en cours"}</h1>
          ...
          <div className="w-full max-w-md bg-slate-950/60 border border-white/5 rounded-2xl p-4 md:p-6 backdrop-blur-md">
            <AnimatePresence mode="wait">
              {!isSubscribed ? (
                <motion.form initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubscribe} className="space-y-3">
                  ...
                  <button type="submit" disabled={isSubmitting} className="w-full py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl font-bold text-xs shadow-lg disabled:opacity-50">
                    {isSubmitting ? (isRtl ? "جاري الحفظ..." : "Inscription...") : (isRtl ? "اشترك الآن مجاناً" : "M'abonner")}
                  </button>
                </motion.form>
              ) : (
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-2 py-4 text-emerald-400 font-bold text-xs">
