...
// Supports: Google, Facebook, GitHub, Microsoft, Yahoo
// (Apple removed: requires a paid Apple Developer Program membership)
...
type ProviderDef = {
  ...
  build: () => OAuthProvider | GithubAuthProvider;
  dark?: boolean;
};
      ...
      <div className="grid grid-cols-2 gap-3">
        {PROVIDERS.map((p) => (
          <motion.button
            ...
            disabled={disabled || loadingProvider !== null}
            title={p.name}
            className={`flex items-center justify-center gap-2.5 py-3 rounded-xl border font-bold text-xs transition-all disabled:opacity-50 ${p.color} ${p.hover} ${p.border} ${p.text} shadow-xs`}
          >
